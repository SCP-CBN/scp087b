...
const Collision Collision::NO_HIT = Collision();

Collision Collision::update(const Line3f& line, float t, const Vector3f& n) {
    if (t < 0.f || t > 1.f) { return NO_HIT; }

    Plane p(n, (line.pointB - line.pointA) * t + line.pointA);
    if (p.normal.dotProduct(line.pointB - line.pointA) >= 0) { return NO_HIT; }
    if (p.onPlane(line.pointA) == Plane::PointRelation::ON) { return NO_HIT; }
...
Collision Collision::sphereCollide(const Line3f& line, float radius, const Vector3f& otherPos, float other_radius) {
    ...
    if (Math::equalFloats(a, 0.f)) { return NO_HIT; }
    ...
    if (Math::equalFloats((line.pointA + lineDir * t).length(), 0.f)) { return NO_HIT; }
...
Collision Collision::edgeTest(const Vector3f& v0, const Vector3f& v1, const Vector3f& pn, const Vector3f& en, const Line3f& line, float radius) {
    ...
    Vector3f tm0Transposed(tm0.x, tm1.x, tm2.x);
    Vector3f tm1Transposed(tm0.y, tm1.y, tm2.y);
    Vector3f tm2Transposed(tm0.z, tm1.z, tm2.z);
    ...
    if (!a) { return NO_HIT; }                    //ray parallel to cylinder
    b = (sv.x * dv.x + sv.z * dv.z) * 2;
    c = (sv.x * sv.x + sv.z * sv.z) - radius * radius;
    d = b * b - 4 * a * c;
    if (d < 0) { return NO_HIT; }                    //ray misses cylinder
    t1 = (-b + sqrt(d)) / (2 * a);
    t2 = (-b - sqrt(d)) / (2 * a);
    t = t1 < t2 ? t1 : t2;
    if (t > 1.f) { return NO_HIT; }    //intersects too far away
    Vector3f i = sv + dv * t;
    Vector3f p = Vectors::ZERO3F;
    if (i.y > v0.distance(v1)) { return NO_HIT; }    //intersection above cylinder
    if (i.y >= 0) {
        p.y = i.y;
    } else {
        //below bottom of cylinder...do sphere test...
        a = dv.dotProduct(dv);
        if (!a) { return NO_HIT; }                //ray parallel to sphere
        b = sv.dotProduct(dv) * 2;
        c = sv.dotProduct(sv) - radius * radius;
        d = b * b - 4 * a * c;
        if (d < 0) { return NO_HIT; }                //ray misses sphere
        t1 = (-b + sqrt(d)) / (2 * a);
        t2 = (-b - sqrt(d)) / (2 * a);
        t = t1 < t2 ? t1 : t2;
        if (t > 1.f) { return NO_HIT; }
        i = sv + dv * t;
    }
...
Collision Collision::triangleCollide(const Line3f& line, float radius, const Vector3f& v0, const Vector3f& v1, const Vector3f& v2) {
    //triangle plane
    Plane p(v0, v1, v2);
...
Collision Collision::triangleCollide(const Line3f& line, float height, float radius, const Vector3f& v0, const Vector3f& v1, const Vector3f& v2) {
    if (height <= radius) {
    ...
    Plane bottomPlane(planeNormal, line.pointA + Vector3f(0.f, -height * 0.5f + radius, 0.f) + planePoint);
    Plane topPlane(planeNormal, line.pointA + Vector3f(0.f, height * 0.5f - radius, 0.f) + planePoint);
