...
class IRoomInfo {
	public:
		...
		virtual RoomInstance* instantiate(CollisionMeshCollection& cmc) const = 0;
...
class RoomInfo : public IRoomInfo {
	...
	public:
		...
		RoomInstance* instantiate(CollisionMeshCollection& cmc) const override {
			PGE_ASSERT(room != nullptr, "Room was not loaded!");
			return new RI(*room, cmc);
		}
